...
- `OWASP ZAP` のような脆弱性診断ツールを使ってチェックする
  - 外部サイトではやらないように（外部のサイトに攻撃すると悪質な場合、最悪逮捕される可能性もあるので取り扱いに注意 👮）
  - もちろん手動で検証する場合も注意
...
例えば、次のような重要な処理を含む機能に対し、罠のリンクを踏ませることで罠を踏んだ利用者の権限で、意図しない処理を行うことができてしまいます。
...
## すぐに上長などに報告しましょう
- （報告する人）ちょっと躊躇するかもしれませんが、強い気持ちを持って報告しましょう💪
  - スルーしないこと
